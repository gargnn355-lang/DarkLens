...
async function readLinksFromDB() {
  ...
  if (error) {
    console.error('Error reading fetched_links:', error);
    return [];
  }
  return data.map(row => row.url);
}

// Add your Supabase Storage bucket name here
  ...
  try {
    ...
    try {
      ...
      if (!uploadError && uploadData) {
        ...
        console.error(`[WARN] Failed to upload screenshot for ${url}:`, uploadError?.message);
      }
    } catch (e) {
...
async function main() {
  while (true) {
    ...
    let driver = await launchTorSelenium();
    for (const url of links) {
      try {
        await crawlLink(driver, url, url); // Pass url as sourceUrl for top-level crawl
      } catch (e) {
        console.error(`[ERROR] Top-level crawl for ${url}:`, e.message);
      }
    }
    try { await driver.quit(); } catch {}
    console.log(`[INFO] Sleeping for ${CRAWL_INTERVAL / 1000}s before next round...`);
